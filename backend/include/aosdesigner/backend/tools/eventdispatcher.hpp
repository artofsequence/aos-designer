...
namespace backend {
	...
	private:
		...
		class EventObservers : public ObserverGroup
		{
			...
			static Connection connection_impl( Signal& signal, TaskType&& task )
			{
				return signal.connect( std::forward<TaskType>(task) ); 
			}
			...
			public:
				...
				Connection on( ObserverType&& observer )
				{
					return connection_impl( *signal, std::forward<ObserverType>(observer) );
				}
			};
		...
		public:
			...
			{
				return connection_impl( m_global_signal, std::forward<ObserverType>(observer) );
			}
			...
			{
				return connection_impl( m_global_signal, [=]( const EventType& ){ observer(); } );
			}

			template< class SourceType, class ObserverType >
			auto connect( const Id<SourceType>& id, ObserverType&& observer ) 
				-> typename std::conditional< true, Connection, decltype( observer( *dummy() ) ) >::type
			{

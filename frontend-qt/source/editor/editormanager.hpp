...
namespace view
{
	...
	public slots:
		...
		void select_editor( backend::EditionSessionId session_id );

		/** Create and register an editor to be displayed to allow edition of the provided edition session. **/
		void add_editor( const backend::EditionSession& edition_session );

		void remove_editor( const backend::EditionSession& edition_session );
		void remove_editor( backend::EditionSessionId session_id );
	...
	private:
		...
		std::map< backend::EditionSessionId, std::unique_ptr<Editor> > m_editor_registry;

		FreeWindowManager& m_window_manager;

		Editor* find_editor( backend::EditionSessionId session_id );
